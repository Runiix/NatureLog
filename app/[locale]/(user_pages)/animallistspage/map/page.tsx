...
import ClientWrapper from "@/app/[locale]/components/listsmap/ClientWrapper";
...
import { getListStats } from "@/app/[locale]/utils/animalLists";
...
export default async function ListMapPage() {
  ...
  const [userNames, stats] = await Promise.all([
    getUsernames(supabase, [...new Set(animalLists.map((list) => list.user_id))]),
    getListStats(supabase, animalLists.map((list) => list.id)),
  ]);

  const animalListsWithUsernames = animalLists.map((list) => ({
    ...list,
